...
db_url = os.environ['DB_URL_POSTGRESQL']
...
async def create_chat(headers: Request):
    ...
    while True:
        ...
        for chats_id in cur.fetchall():
            ...
            break
    cur.execute('INSERT INTO chat_info (chat_id, creation_date) VALUES (%s, %s)', (chat_id, datetime.now()))
    conn.commit()
    cur.close()
    conn.close()
...
async def create_title(headers: Request, chat_id: str = Form(...), generate: bool = Form(...), message: str = Form(...)):
    ...
    if generate == True:
        ...
        cur.execute('UPDATE chat_info SET title_name = %s WHERE chat_id = %s', (response, chat_id,))
        conn.commit()
        cur.close()
        conn.close()
        return response
    else:
        cur.execute('UPDATE chat_info SET title_name = %s WHERE chat_id = %s', (message, chat_id,))
        conn.commit()
        cur.close()
        conn.close()
        return message 
...
async def delete_chat(headers: Request, chat_id: str):
    ...
    cur.execute("DELETE FROM chat_info WHERE chat_id = %s", (chat_id,))
    conn.commit()
    cur.close()
    conn.close()
...
async def req(headers: Request, chat_id: str = Form(...), message: str = Form(...), model: str = Form(...), provider: str = Form(...), web_search: bool = Form(...), image_base64: str = Form(None), image: UploadFile = File(None)):
    ...
    messages = []
    cur.execute("SELECT role, message FROM messages WHERE chat_id = %s", (chat_id,))
    ...
    cur.execute('INSERT INTO messages (chat_id, message_id, role, message) VALUES (%s,%s,%s,%s)', (chat_id, len(messages)+1, 'user', image_base64_format+message,))
    conn.commit()
    ...
    async def response_send(messages, model, chat_id, provider, **kwargs):
        ...
        try:
            ...
            conn = psycopg2.connect(db_url)
            cur = conn.cursor()
            cur.execute('INSERT INTO messages (chat_id, message_id, role, message) VALUES (%s,%s,%s,%s)', (chat_id, len(messages)+1, 'assistant', text,))
            conn.commit()
            cur.close()
            conn.close()    
...
async def save_settings(headers: Request, model: str, provider: str, web_search: bool, generated_title: bool):
    ...
    conn = psycopg2.connect(db_url)
    cur = conn.cursor()
    cur.execute("UPDATE settings SET value = %s WHERE key = 'model'", (model,))
    cur.execute("UPDATE settings SET value = %s WHERE key = 'provider'", (provider,))
    cur.execute("UPDATE settings SET value = %s WHERE key = 'web_search'", (web_search,))
    cur.execute("UPDATE settings SET value = %s WHERE key = 'generated_title'", (generated_title,))
    conn.commit()
    cur.close()
    conn.close()
    return {'status': 'success'}
